...
CREATE TABLE engine_result (
    ...
    hit_online_rules     TEXT NULL COMMENT '命中上线规则集合',
    hit_mock_rules       TEXT NULL COMMENT '命中模拟规则集合',
...
- **引擎服务**:
  ...
  - **GroovyShell执行相关groovy字符串脚本返回true或者false**
  - **整理命中的上线规则和模拟规则集合，按照状态上线和分数排序降序，第一个规则处置方式返回给业务方结果**:
  - **异步发送mq消息 消息内容是业务结果**
    - **保存mysql业务结果数据**
    - **通过canal监听mysql表 同步到保存到es**
    - **遍历命中的上线规则 执行处罚**
      - **处罚中心 组装数据、处罚接口的json报文和特征值、生成处罚记录** 
      - **定时任务遍历 处罚记录 根据status=0和retry<=3**
